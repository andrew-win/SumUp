...
class AiSummaryResponseMapper @Inject constructor() : SummaryResponseMapper {

    override fun parseSingle(jsonResponse: String, content: String): SummaryResult.Single {
    ...
    override fun parseCompare(jsonResponse: String, content: String): SummaryResult.Compare {
    ...
    override fun parseFeed(jsonResponse: String, content: String): SummaryResult.Digest {
        val parsed = AiJsonResponseParser.parseSummary(jsonResponse)
        val sourceRefs = extractStructuredSourceRefs(content)
        val result = toSummaryResult(parsed, sourceRefs)
    ...
    override fun parseQuestion(jsonResponse: String, content: String, question: String): SummaryResult.QA {
                ...
                themes = json.themes.map { theme ->
                    DigestTheme(
                        ...
                        items = theme.items.map { item ->
                            SummaryItem(
                                text = item.title ?: "",
                                sources = (item.sourceIds + listOfNotNull(item.sourceId)).mapNotNull { sourceRefs[it] }
                                    .distinct()
                            )
                        }
                    )
