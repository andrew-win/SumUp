    ...
    private fun toImportedGroup(groupMap: Map<*, *>): ImportedSourceGroup? {
        ...
        val sources = sourceMaps.mapNotNull { item ->
            ...
            ImportedSource(
                ...
                footerPattern = (map["footerPattern"] as? String)?.trim()
                    ?.takeIf { it.isNotEmpty() },
                footerPatternCheckedAt = (map["footerPatternCheckedAt"] as? Number)?.toLong() ?: 0L,
                titleSelector = (map["titleSelector"] as? String)?.trim()
                    ?.takeIf { it.isNotEmpty() },
                postLinkSelector = (map["postLinkSelector"] as? String)?.trim()
                    ?.takeIf { it.isNotEmpty() },
                descriptionSelector = (map["descriptionSelector"] as? String)?.trim()
                    ?.takeIf { it.isNotEmpty() },
