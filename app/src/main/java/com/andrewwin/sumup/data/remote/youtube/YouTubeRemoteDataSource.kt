package com.andrewwin.sumup.data.remote.youtube
        ...
        try {
            ...
            okHttpClient.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    return@withContext youtubeParser.parse(
                        inputStream = bodyBytes.inputStream(),
                        sourceId = source.id,
                        oldestAllowedPublishedAt = oldestAllowedPublishedAt
                    )
            ...
            NewsParsingLogger.error(e) {
                "youtube_feed_error sourceId=${source.id} url=${NewsParsingLogger.safeUrl(source.url)} " +
