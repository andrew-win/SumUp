...
class RemoteArticleDataSource @Inject constructor(
    ...
    private fun fetchTelegramPageArticles(
        ...
        return try {
            okHttpClient.newCall(request).execute().use { response ->
                ...
                val html = response.body?.string() ?: return TelegramPageResult.empty(statusCode = response.code, requestMs = requestMs)
    ...
    private fun fetchYouTubeArticles(
        sourceId: Long,
        url: String,
        oldestAllowedPublishedAt: Long?,
        latestKnownArticleUrl: String?
        ...
        return try {
            ...
            okHttpClient.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    if (bodyBytes != null) {
                        val metadata = youtubeParser.parseFeedMetadata(
                            inputStream = bodyBytes.inputStream(),
                            oldestAllowedPublishedAt = oldestAllowedPublishedAt,
