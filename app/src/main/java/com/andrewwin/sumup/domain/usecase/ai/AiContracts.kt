package com.andrewwin.sumup.domain.usecase.ai

object AiJsonContract {
    const val HEADLINE = "headline"
    const val SUMMARY = "summary"
    const val ITEMS = "items"
    const val THEMES = "themes"
    const val TITLE = "title"
    const val BULLETS = "bullets"
    const val SOURCE = "source"
    const val SOURCE_URL = "source_url"
    const val SOURCE_ID = "source_id"
    const val EMOJIS = "emojis"
    const val COMMON = "common"
    const val DIFFERENT = "different"
    const val QUESTION = "question"
    const val ANSWER = "answer"
    const val SHORT_ANSWER = "short_answer"
    const val DETAILS = "details"
    const val STATEMENTS = "statements"
    const val TEXT = "text"
    const val SOURCES = "sources"
    const val COMMON_FACTS = "common_facts"
    const val COMMON_TOPIC = "common_topic"
    const val UNIQUE_DETAILS = "unique_details"
    const val FALLBACK_MESSAGE = "fallback_message"
}


